...
def calculate_loss(logit, target, pad_idx, smoothing=False):
    ...
    '''
    if smoothing:
        # logit无后继，label无前驱,压扁免循环
        logit = logit[..., :-1, :].contiguous().view(-1, logit.size(2)) # 前n预测后n+1，
        target = target[..., 1:].contiguous().view(-1) # ...表示任意数量的前置维度
        # 标签平滑，真实1->0.9剩下0.1均分
        eps = 0.1
        ...
        non_pad_mask = target.ne(pad_idx) # no equal, != 为True
        loss = -(one_hot * log_prb).sum(dim=1) # 交叉熵的展开形式：-Σ(y_true * log(y_pred))
